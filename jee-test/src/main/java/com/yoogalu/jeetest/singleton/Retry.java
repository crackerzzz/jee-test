	...
	public Object interceptAndRetryIfNeeded(final InvocationContext ic) {
		...
		if (policy != null) {
			...
			throw new IllegalStateException(String.format(
					"Method '%s' from class '%s' should be annotated with @RetryPolicy", ic.getMethod().getName(), ic
		...
		do {
			try {
				if (retryCount > 0) {
					System.out.println(String.format("Method '%s' from class '%s' is being retried by thread %s", ic
							.getMethod().getName(), ic.getTarget().getClass().getName(), Thread.currentThread()
									.getName()));
				}
				result = ic.proceed();
				failed = false;
